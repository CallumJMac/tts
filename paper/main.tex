...
\textsc{Concat} with random selection at $n{\geq}3$ shows the clearest instability: failure rate (WER$>50\%$) of $2.7\%$ at $n{=}3$ and $1.3\%$ at $n{=}5$, with rare extreme outliers inflating means.
By contrast, \textsc{embed} has 0\% failures under this threshold across all tested configurations.
...
The instability risk is concentrated in \textsc{concat} with random selection, particularly at larger reference counts.
For \textsc{concat}, selecting longer references improves both fidelity and robustness.
...
\textbf{Limitations.}
We evaluate a single TTS architecture (Qwen3-TTS~0.6B) on a single dataset (LibriTTS-R) in English only.
